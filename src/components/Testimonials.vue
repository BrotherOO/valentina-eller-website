<template>
  <section class="py-20 bg-gradient-to-br from-[#FFF5F2] to-white overflow-hidden">
    <div class="container mx-auto px-4 max-w-6xl text-center">
           ...
           <Transition name="fade" mode="out-in">
             <div :key="currentIndex" class="w-full flex justify-center">
                <div class="bg-white p-8 md:p-12 rounded-3xl shadow-xl relative w-full text-center flex flex-col items-center border border-gray-50 h-full min-h-[420px]">
                     ...
                     <!-- Stars -->
                     <div class="flex justify-center gap-1 mb-6 text-yellow-400 mt-6 w-full">
                        <svg v-for="n in 5" :key="n" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
      ...
      <!-- Logic: Mobile Arrows (Below) for usability on small screens if side arrows don't fit? Or just Dots? User said "Tap Targets"... keeping Dots and maybe Mobile Arrows below if side arrows hidden -->
      <div class="flex md:hidden justify-center gap-6 mt-8">
          <button @click="prevSlide" class="p-3 rounded-full bg-white shadow-md text-gray-500 hover:text-accent-warm">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
              </svg>
          </button>
           <div class="flex items-center gap-2">
            <button v-for="(_, index) in reviews" :key="index" @click="currentIndex = index" :class="['h-2 rounded-full transition-all duration-300', currentIndex === index ? 'bg-accent-warm w-6' : 'w-2 bg-gray-300']" :aria-label="'Gehe zu Bewertung ' + (index + 1)">
            </button>
           </div>
          <button @click="nextSlide" class="p-3 rounded-full bg-white shadow-md text-gray-500 hover:text-accent-warm">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
              </svg>
          </button>
      ...
      <!-- Desktop Dots (Below) -->
      <div class="hidden md:flex justify-center gap-3 mt-10">
        <button v-for="(_, index) in reviews" :key="index" @click="currentIndex = index" :class="['h-2 rounded-full transition-all duration-300', currentIndex === index ? 'bg-accent-warm w-8' : 'w-2 bg-gray-300 hover:bg-[#F8A08C]/50']" :aria-label="'Gehe zu Bewertung ' + (index + 1)">
        </button>
      </div>
